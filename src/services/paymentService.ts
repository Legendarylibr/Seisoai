...
export const getTokenBalance = async (
  ...
  try {
    const contract = new ethers.Contract(tokenAddress, ERC20_ABI, provider) as Contract;
...
export const transferTokens = async (
  ...
  try {
    const contract = new ethers.Contract(tokenAddress, ERC20_ABI, signer) as Contract;
    ...
    const err = error as Error;
    logger.error('Error transferring tokens', { error: err.message });
 ...
 */
export const removePaymentToken = (chainId: string, symbol: string, walletType: string = 'evm'): void => {
  if (walletType === 'solana') {
