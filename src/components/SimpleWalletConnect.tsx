...
// Win95 style button
const Win95Btn: React.FC<Win95BtnProps> = ({ children, onClick, disabled, className = '', style = {} }) => (
  <button
    onClick={onClick}
    disabled={disabled}
...
const ErrorMessageWithLink = memo<ErrorMessageWithLinkProps>(({ message }) => {
...
ErrorMessageWithLink.displayName = 'ErrorMessageWithLink';
...
const WalletOption = memo<WalletOptionProps>(({ wallet, onClick, isConnecting, disabled }) => (
  <button 
...
const WalletModal = memo<WalletModalProps>(({ isOpen, onClose, onSelectWallet, isConnecting, connectingWallet, error }) => {
...
const SimpleWalletConnect: React.FC = () => {
  ...
  const handleSelectWallet = async (walletId: string) => {
    ...
    try { 
      ...
      setModalError(err instanceof Error ? err.message : 'Connection failed'); 
    } finally { 
