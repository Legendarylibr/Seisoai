...
const findProvider = (predicate) => {
  try {
    ...
    if (eth.providers?.length) {
      const found = eth.providers.find(p => { try { return predicate(p); } catch { return false; } });
      if (found) return found;
    }
    try { if (predicate(eth)) return eth; } catch {}
    return null;
  } catch { return null; }
};
...
export const SimpleWalletProvider = ({ children }) => {
  ...
  const fetchCredits = useCallback(async (walletAddress, _retries, force = false) => {
    ...
    try {
      ...
      logger.error('Failed to fetch credits', { error: e.message });
      return 0;
    }
  }, [credits]);
  ...
  const checkNFTStatus = useCallback(async (walletAddress) => {
    ...
    try {
      ...
      if (result.creditsGranted > 0) await fetchCredits(walletAddress, 0, true);
    } catch (e) {
  ...
  const connectWallet = useCallback(async (selectedWallet = 'metamask') => {
    try {
      setIsLoading(true);
      setError(null);
      let provider = null, walletAddress = null;

      const getEvmAddress = async (p) => (await p.request({ method: 'eth_requestAccounts' }))[0];

      switch (selectedWallet) {
        case 'metamask': {
          const isReal = p => p?.isMetaMask && !p.isRabby && !p.isCoinbaseWallet && !p.isBraveWallet && !p.isTrust && !p.isPhantom;
          const eth = safeGetEthereum();
        ...
        case 'phantom-evm': {
          ...
          walletAddress = await window.phantom.ethereum.request({ method: 'eth_requestAccounts' }).then(a => a[0]);
          setWalletType('evm');
          break;
        }
        case 'phantom': {
      ...
      checkNFTStatus(walletAddress).catch(() => {});
    } catch (e) {
      setError(e.message);
      throw e;
    } finally {
  ...
  }, [fetchCredits, checkNFTStatus]);

  const disconnectWallet = useCallback(async () => {
    if (walletConnectProviderRef.current) {
      try { await walletConnectProviderRef.current.disconnect(); } catch {}
