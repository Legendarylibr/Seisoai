 ...
 * @param {string|Buffer} videoInput - Video URL or buffer
 * @param {Object} options - Options
 * @param {string} options.format - Output format: 'mp4', 'webm' (default: 'mp4')
 * @returns {Promise<Buffer>} - Cleaned video buffer (no metadata)
 */
export const stripVideoMetadata = async (videoInput, options = {}) => {
  ...
  try {
    ...
    logger.error('Failed to strip video metadata', { error: error.message });
 ...
 * @param {Object} options - Options
 * @returns {Promise<Buffer>} - Cleaned video buffer
 */
export const stripVideoMetadataFromUrl = async (videoUrl, options = {}) => {
 ...
 * @returns {Promise<boolean>} - True if FFmpeg is available
 */
export const isVideoMetadataCleaningAvailable = async () => {
  if (ffmpegAvailable === null) {
    ffmpegAvailable = await checkFFmpegAvailable();
  }
