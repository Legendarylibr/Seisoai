 ...
 *
 * Run with: node scripts/migrate-to-collections.js
 */
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (user.generationHistory && user.generationHistory.length > 0) {
        const generations = user.generationHistory.map((gen, idx) => ({
          userId,
      ...
      if (user.gallery && user.gallery.length > 0) {
        const galleryItems = user.gallery.map((item, idx) => ({
          userId,
      ...
      if (user.paymentHistory && user.paymentHistory.length > 0) {
        const payments = user.paymentHistory.map((payment, idx) => ({
          userId,
